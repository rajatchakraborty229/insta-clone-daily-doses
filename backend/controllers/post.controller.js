...
export const addNewPost = async (req, res) => {
    try {
        ...
        return res.status(201).json({
            message: "New post added sucessfully",
            success: true,
            post
        })
    } catch (error) {
        console.log("Error in addNewPost controller", error.message);
...
export const likePost = async (req, res) => {
    try {
        ...
        await Post.updateOne({ _id: postId }, { $addToSet: { likes: likekarneWalaUserId } });

        const user = await User.findById(likekarneWalaUserId).select('username profilePicture');

        if (post.author && post.author.toString() !== likekarneWalaUserId) {
            const notification = {
                type: "like",
...
export const dislikedPost = async (req, res) => {
    try {
        ...
        await Post.updateOne({ _id: postId }, { $pull: { likes: likekarneWalaUserId } });
        ...
        console.error("Error in dislikePost controller:", error.message);
        return res.status(500).json({ message: "Internal server error", success: false });
    }
};
...
export const getCommentsOfPost = async (req, res) => {
    try {
        ...
        console.log("error in getCommentsofPost controller", error.message);
    }
}
